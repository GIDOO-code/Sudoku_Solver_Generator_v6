...
//			if( FreeB0.BitCount() > 2 )  return;	// ... necessary !
		...
		private void Test_SE_Rule11_EscapeCells_nonSCells( USExocet SExo, bool debugPrint=false ){
			...
			int hBase = SExo.rcStem.B();
			foreach( int no in FreeBnon.IEGet_BtoNo() ){
				UInt128 Escape81 = FreeCell81b9[no].DifSet(SExo.Base81) & SExo.EscapeCells;
				
				UInt128 rcB = Escape81 & HouseCells81[hBase+18];
				if( rcB.BitCount() == 1 ) _Test_SE_Rule11_sub(no,rcB);

				rcB = Escape81.DifSet( HouseCells81[hBase+18] ) ;
				if( rcB.BitCount() == 1 ) _Test_SE_Rule11_sub(no,rcB);
			}
						void _Test_SE_Rule11_sub( int no, UInt128 rcB ){
							int rc = rcB.FindFirst_rc();
							UCell UC = pBOARD[rc];
							if( UC.No!=0 )  return;
							string st = $"[Rule-11] Base({SExo.stBase})#{no+1} are Fixed, then {rcB.TBScmp()}#{no+1} is positive.";
							UC.No = -(no+1);
						}	
