...
namespace GNPX_space{
    public partial class NXGCellLinkGen: AnalyzerBaseV2{
        ...
        // https://gidoo-code.github.io/Sudoku_Solver_Generator_v5/page46.html
        ...
        public bool Color_Trap( ){
            ...
            for(int no=0; no<9; no++ ){
                int noB=(1<<no);
                foreach( UInt128[] CRL in _Coloring(no) ){                //_Coloring: cells_network connected by strong-links

                    UInt128 HitB = UInt128.Zero;
                    UInt128 ELM  = FreeCell81b9[no] .DifSet(CRL[0] | CRL[1]);  //ELM:Eliminate cells from CRL_cells with digit(no)
                    foreach( var rc in ELM.IEGet_rc() ){                //rc:Candidate cell  
                        UInt128 CNb = ConnectedCells81[rc];                     //Cells not included in Network
                        if( (CNb&CRL[0]).IsZero() ) continue;               //Eliminate Cells related to Network
                        if( (CNb&CRL[1]).IsZero() ) continue;
                        HitB = HitB.Set(rc);                                 //solution cell
                    }
                    ...
                    if( HitB.IsNotZero() ){ //===== found =====
                        ...
                        Result = SolMsg;
                        if( SolInfoB ){
                            string st1 = $"\r group 1 : {CRL[0].ToRCStringComp()}";
                            st1 += $"\r group 2 : {CRL[1].ToRCStringComp()}";
                            st1 += $"\r eliminate : {HitB.ToRCStringComp()}#{no+1}";

                            ResultLong = SolMsg + st1;
                        
                            Color Cr =_ColorsLst[1];
                            Color Cr1= Cr * 0.7f;                  //Color.FromArgb(120,Cr.R,Cr.G,Cr.B);  
        ...
        public bool Color_Wrap( ){
            ...
            for(int no=0; no<9; no++ ){
                int noB=(1<<no);
                UInt128 BD0 = FreeCell81b9[no];

                foreach( UInt128[] CRL in _Coloring(no) ){                // _Coloring: cells_network connected by strong-links
                    if( CRL[0].BitCount()<=1 || CRL[1].BitCount()<=1)  continue;

                    UInt128 ELM = BD0.DifSet( CRL[0] | CRL[1] );                    // ELM:Elements not included in the network
                    if( ELM.BitCount()==0 ) continue;

                    for(int k=0; k<2; k++ ){
                        
                        for(int h=0; h<27; h++ ){
                            if( (CRL[k]&HouseCells81[h]).BitCount() < 2 ) continue;
                            ...
                            Result=SolMsg;
                            if( SolInfoB ){
                                string st1 = $"\r group 1 : {CRL[1-k].ToRCStringComp()}";
                                st1 += $"\r group 2 : {CRL[k].ToRCStringComp()}";
                                st1 += $"\r eliminate : {CRL[k].ToRCStringComp()}#{no+1}";
        ...
        public bool MultiColor_Type1( ){
            ...
            for(int no=0; no<9; no++ ){
                int noB=(1<<no);
                UInt128 BD0 = FreeCell81b9[no];
                List<UInt128[]> MCRL = _Coloring(no:no, minSize:2, bMulti:true).ToList();

                if( MCRL==null || MCRL.Count()<2 ) continue;

                var  cmb = new Combination( MCRL.Count(), 2 );
                while( cmb.Successor() ){
                    UInt128[] CRLa = MCRL[cmb.Index[0]];      // group_A
                    UInt128[] CRLb = MCRL[cmb.Index[1]];      // group_B

                    for(int na=0; na<2; na++ ){
                        UInt128 HCRLa = new UInt128();
                        foreach(var rc in CRLa[na].IEGet_rc()) HCRLa |= ConnectedCells81[rc]; // HCRLa: cell group connected to group_A[na]

                        for(int nb=0; nb<2; nb++ ){
                            if( (HCRLa&CRLb[nb]).IsZero() ) continue;
                            // HCRLa and group_B[nb] are weakly connected

                            UInt128 ELMtry = BD0 .DifSet( (CRLa[na] | CRLb[nb] | CRLa[1-na] | CRLb[1-nb]) ); // Cells unrelated to group_A and group_A
                            if(ELMtry.BitCount() ==0) continue;
                            
                            bool found=false;
                            UInt128 ELM=new UInt128();
                            foreach( var rc in ELMtry.IEGet_rc() ){
                                if( !ConnectedCells81[rc].IsHit(CRLa[1-na]) ) continue;
                                if( !ConnectedCells81[rc].IsHit(CRLb[1-nb]) ) continue;
                                pBOARD[rc].CancelB=noB; ELM=ELM.Set(rc); found=true;
                            }
                            if(found){
                                SolCode = 2;
                                ...
                                if( SolInfoB ){
                                    ...
                                    string st1 = "";
                                    for(int k=0; k<2; k++){
                                        UInt128[] CRLX = MCRL[cmb.Index[k]];
                                        Color Cr1=_ColorsLst[k*2];
                                        Color Cr2 = Cr1 * 0.7f;          
                                        CRLX[1-k].IE_SetNoBBgColor( pBOARD, noB, AttCr, Cr1 );
                                        CRLX[k].IE_SetNoBBgColor(   pBOARD, noB, AttCr, Cr2 );

                                        st1 +=  $"\r group {k+1}-A: {CRLX[0].ToRCStringComp( $"#{no+1}" )}";
                                        st1 +=  $"\r group {k+1}-B: {CRLX[1].ToRCStringComp( $"#{no+1}" )}";
                                    }
                                    st1 +=  $"\r eliminate : {ELM.ToRCStringComp( $"#{no+1}" )}";
        ...
        public bool MultiColor_Type2( ){
            ...
            for(int no=0; no<9; no++ ){
                int noB=(1<<no);
                List<UInt128[]> MCRL = _Coloring(no:no,minSize:1,bMulti:true).ToList();
                if(MCRL==null || MCRL.Count()<2)  continue;

                var prm = new Permutation( MCRL.Count(), 2 );
                while(prm.Successor()){
                    UInt128[] CRLa = MCRL[prm.Index[0]];
                    UInt128[] CRLb = MCRL[prm.Index[1]];

                    //*** Choose network "a"
                    UInt128 HCRL0=new UInt128(), HCRL1=new UInt128();
                    foreach( var rc in CRLa[0].IEGet_rc() ) HCRL0 |= ConnectedCells81[rc];  //all elements connected to a[0] -> HCRL0
                    foreach( var rc in CRLa[1].IEGet_rc() ) HCRL1 |= ConnectedCells81[rc];  //all elements connected to a[1] -> HCRL1
                    ...
                    for(int nb=0; nb<2; nb++ ){
                        ...
                        if( SolInfoB ){
                            ...
                            string st1 = $"\r group 1-A: {CRLa[0].ToRCStringComp( $"#{no+1}" )}";
                            st1 += $"\r group 1-B: {CRLa[1].ToRCStringComp( $"#{no+1}" )}";
    
                            st1 +=  $"\r group 2-A: {CRLb[1-nb].ToRCStringComp( $"#{no+1}" )}";
                            st1 += $"\r group 2-B: {CRLb[nb].ToRCStringComp( $"#{no+1}" )}";   
        
                            st1 +=  $"\r eliminate : {CRLb[nb].ToRCStringComp( $"#{no+1}" )}";
...
#region Abstraction code version
        ...
        private IEnumerable<UInt128[]> _Coloring( int no, int minSize=1, bool bMulti=false ){ 
            ColoringQueue QX = new ColoringQueue(FreeCell81b9,no,bMulti);
            ...
            while( (_rc1=QX.FindFirst_rc()) >= 0 ){     // _rc1:Starting point cell for coloring
                ...
                if( QX.CRL[0].BitCount()>=minSize && QX.CRL[1].BitCount()>=minSize )  yield return QX.CRL;
        ...
        private class ColoringQueue{
            ...
            private UInt128     BPno;                   // Bit representation of the cells with #no as a candidate
            public  UInt128[]   CRL;                    // colored network
            private bool        bMulti;
            public int          Count => Que.Count();

            public ColoringQueue( UInt128[] pFreeCell81b9, int no, bool bMulti=false  ){
                this.Que  = new Queue<int>();
                this.BPno = pFreeCell81b9[no];
                this.bMulti = bMulti;
                CRL = new UInt128[2]; CRL[0]=new UInt128(); CRL[1]=new UInt128();
            }
            ...
            public int FindFirst_rc(){
                ...
                if( _rc1>=0 ){     
                    ...
                    CRL[0] = CRL[0].Set(_rc1);                 // record first node(cell) in network
                }
                return _rc1;
            }
            ...
            public void CheckAndEnqueue( UCellLink P, int _color2 ){ 
                ...
                if( !(CRL[0]|CRL[1]).IsHit(_rc2) ){    // If P is not connected,           
                    CRL[_color2] = CRL[_color2].Set(_rc2);          //   expand the network.   
...
#region Naive code version
        private IEnumerable<UInt128[]> _Coloring_Basic( int no ){
            UInt128[] CRL=new UInt128[2];               
            CRL[0]=new UInt128(); CRL[1]=new UInt128(); 
            UInt128 BPno = FreeCell81b9[no];
            int  rc1;
