...
namespace GNPX_space{
    ...
    public partial class AnLSTechGen: AnalyzerBaseV2{
        ...
        public bool Finned_SueDeCoqEx1( ){
	...
	// ----- << 1 >> -----
				...
				foreach( var (ALS_A,RCC_SvsA) in IEGet_ALS_as_Leaf( fALS.ALSList, Stem) ){
					if( RCC_SvsA.BitCount() != 1 )  continue;

					bool RCC_Cells_Cond = (Stem.FreeB.BitCount() - 1) == Stem.Size; //RCC and cell number conditions.
					if( !RCC_Cells_Cond )  continue;

					// search Fin connected to ALS_A.
					int noBFin = (Stem.FreeB & ALS_A.FreeB).DifSet(RCC_SvsA);
					if( noBFin == 0 )  continue;
						//WriteLine( $"@#- Stem.ID:{Stem.ID}  ALS_A.ID:{ALS_A.ID}" );				
						//WriteLine( $"\n\n\n@#- Stem:{Stem}\n\n ALS_A:{ALS_A}\nRCC_SvsA:{RCC_SvsA.ToBitString(9)}" );
						//if(debugPrint)  WriteLine( $"\n\n===  ALS_A:{ALS_A}\n RCC_SvsA:{RCC_SvsA.ToBitString(9)}" );

					UInt128   rcBPattern = Stem.bitExp | ALS_A.bitExp;
					foreach( int noF in noBFin.IEGet_BtoNo(9) ){
						UInt128 rcBbaseSet = Stem.BitExp9[noF] | ALS_A.BitExp9[noF];
						...
						// ----- Solution Found -----------------------------------------------------
						bool solFound = Finned_SDCEx1_SolResult( Stem, ALS_A, RCC_SvsA, noF, rcBCoverSet );
		...
		private bool Finned_SDCEx1_SolResult( UAnLS Stem, UAnLS ALS_A, int RCC_SvsA, int noF, UInt128 rcBCoverSet ){
			...
			ALS_A.bitExp.IE_SetNoBBgColor(pBOARD, 0x1FF, AttCr, SolBkCr20);
			...
			Result     = $"Finned_SueDeCoqEx1 {st_Stem.Replace(" #","#")} / {st_ALS_A.Replace(" #","#")}";

			st_ALS_A += $"  RCC;#{RCC_SvsA.ToBitStringN(9)}";
			string st2 = $"Finned_SueDeCoqEx1\n\n   stem AnLS : {st_Stem}\n   1nd ALS : {st_ALS_A}\n\n      exclude:{st_exclude}";
