...
namespace GNPX_space{
    ...
    public partial class Senior_Exocet_TechGen: AnalyzerBaseV2{
		...
		// Type:base : Starting point for Senior Exocet development.
		//			   This version is to ensure that "Junior Exocet is included."
	//	public bool SExocet_Basic()		 => Exocet_General( ExoControl:"name:SE type:Basic"  );
        ...
        public bool Exocet_General( string ExoControl ){
			...
			foreach( var SExo in IE_SE_ph1_Set_BasicForm1_Set_BasicForm( ExoControl:ExoControl, debugPrint:false) ){			
				//if( SExo.dir!=1 || SExo.rcStem!=35 )  continue;			// ===== SE_Nxt Debug =====
				...
				foreach( var _ in IE_SE_ph2_Get_TargetObject_SLine( SExo, debugPrint:false) ){
					...
					//int FreeBPos = SExo.FreeB .DifSet(FreeB_Companions);
					//if( FreeBPos.BitCount() < 2 )  continue;
					//foreach( var P in SExo.Base81.IEGet_UCell(pBOARD) )  P.CancelB = P.FreeB&FreeB_Companions;
					...
					int[] Coveded_noB = SExo.CoverLine_by_Size;
					int[] Coveded4size = Coveded_noB.ToList().ConvertAll(p => p.BitCount() ).ToArray();

					// ... validation
					if( SExo.ExocetName.Contains("JE1") || SExo.ExocetNamePlus=="SE_Single" ){ // ... JE1
						if( Coveded4size[3]!=1 || SExo.FreeB .DifSet(Coveded_noB[2] | Coveded_noB[3]) > 0 )  goto LClear;
					}
					else if(SExo.ExocetName.Contains("JE2") ){
						if( Coveded4size[0] > 0 )  goto LClear;
						if( Coveded4size[2] < 2 )  goto LClear;					
					    if( Coveded4size[3] > 0 )  goto LClear;
					}
					else{	// ...  SE
					    if( Check_CoverLine(SExo, Coveded_noB) is false  ) goto LClear;
					}
			...
			return false;	
						...
						bool Check_CoverLine( USExocet SExo, int[] Coveded_noB ){
							int[] Coveded4size = Coveded_noB.ToList().ConvertAll(p => p.BitCount() ).ToArray();

							if( Coveded4size[0] > 0 )  return false;
							if( Coveded4size[2] < 2 )  return false;
							if( Coveded4size[3] > 0 )  return false;

							UCoverLine ExG1=SExo.ExG1, ExG2=SExo.ExG2;
								//WriteLine( $"**CoverLines (p:Parallel x:Cross){SExo.stCoverLines}" );
							int Coveded2 = Coveded_noB[2];		// Candidate digits covered by 2-CoverLine

							//Fail if there are any base digits that are not covered by 2-CL
							if( SExo.Base81.IEGet_UCell(pBOARD).Any(u=>(u.FreeB&Coveded2)==0) )  return false;	

							// The candidate digits of the Target is the Base-Digits and is included in the SLIne.
							int  h_CL1=ExG1.h, h_CL2=ExG2.h, exclude1=Coveded2, exclude2=Coveded2;
